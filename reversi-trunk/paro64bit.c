...
static void displayMove(int move)
{
    printf("%c%d\n",
        (char)(move % MAXX) + 'a', move / MAXY + 1); //printing off move which is the number then % 8 (value from 0 to 7), then adds thta to the char a
    //dividing move by number 1 to 8
    ...
    BITSET64 c, BITSET64 u, int noPlies, int o, int minscore, int maxscore)
{
    ...
    else
    {
        ...
        int i, move_score, move_index, positoons_explored;

        for (i = 0; i < noOfMoves; i++)
        {
            printf("parent waiting for a result\n");
            mailbox_rec(barrier, &move_score, &move_index, &postions_explored);
